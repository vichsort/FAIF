...
class _BuscaUnificadaPageState extends State<BuscaUnificadaPage> {
  ...
  Future<List<ResultadoBusca>> buscarDeputados(String termo) async {
    ...
    if (resp.statusCode == 200) {
      ...
      return data.map((d) => ResultadoBusca(
        titulo: d['nome'] ?? '—',
        descricao: 'Partido: ${d['siglaPartido'] ?? ''} | UF: ${d['siglaUf'] ?? ''}',
        link: d['uri'] ?? '',
      )).toList();
    }
    return [];
  ...
  Future<List<ResultadoBusca>> buscarEmendas(String termo) async {
    ...
    if (resp.statusCode == 200) {
      ...
      return data.map((e) => ResultadoBusca(
        titulo: e['nomeAutor'] ?? '—',
        descricao: 'Emenda: ${e['numeroEmenda'] ?? ''} – Ano: ${e['ano'] ?? ''}',
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Busca Unificada")),
      body: Column(
        children: [
          Padding(
